...
export default {
    ...
    methods: {
        ...
        {
            this.loading=true
            axios.get('https://sistemkedairuncit.onrender.com/item/bread')
            .then(response=> {
...
<template>
  ...
  <div class="w-10/12 grid grid-cols-5 gap-32 gap-y-5 mx-10 text-center max-lg:grid-cols-3 max-lg:gap-10 max-lg:ml-16 max-sm:grid-cols-2 max-sm:ml-6 max-sm:gap-5 " >
    <RouterLink to="" v-for="item in item" v-bind:key="item.idItem" @click="toggleDialog(item)">
        <div class="bg-teal-500 text-white w-48  rounded-2xl p-1 ease-in-out duration-500 hover:scale-110 max-sm:w-[120px]">
